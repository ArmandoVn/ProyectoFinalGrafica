...
float x = 0.0f;
float y = 0.0f;
float movAuto_z, movAuto_x = 0.0f;
float rotateAuto_y = 90.0f;
int estado = 0;
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		tmp = model = glm::translate(model, glm::vec3(movAuto_x, -1.75f, movAuto_z));
		...
		model = glm::scale(model, glm::vec3(0.01f, 0.01f, 0.01f));
		staticShader.setMat4("model", model);
		llanta.Draw(staticShader); // Llanta delantera izquierda

		model = glm::translate(tmp, glm::vec3(-0.85f, 0.25f, 1.29f));
		model = glm::scale(model, glm::vec3(0.01f, 0.01f, 0.01f));
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.01f));
		staticShader.setMat4("model", model);
		llanta.Draw(staticShader); // Llanta delantera derecha

		model = glm::translate(tmp, glm::vec3(-0.85f, 0.25f, -1.45f));
		model = glm::scale(model, glm::vec3(0.01f, 0.01f, 0.01f));
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.01f));
		staticShader.setMat4("model", model);
		llanta.Draw(staticShader); // Llanta trasera derecha

		model = glm::translate(tmp, glm::vec3(0.85f, 0.25f, -1.45f));
		model = glm::scale(model, glm::vec3(0.01f, 0.01f, 0.01f));
		staticShader.setMat4("model", model);
		llanta.Draw(staticShader); // Llanta trasera izquierda
		// -------------------------------------------------------------------------------------------------------------------------
		// Termina Escenario
		// -------------------------------------------------------------------------------------------------------------------------
